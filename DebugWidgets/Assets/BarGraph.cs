...
/// BarGraph widgets are GameObjects that have one of these as a component, as well
as an Image(like a sprite but different)
...
public class BarGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // TODO: If we're going to display signed numbers, then move the bar to the
        middle of the widget
    // and set signedDisplay to true.
    //
    // You can figure out if we're displaying signed numbers by looking at Min
and / or Max.
    // Do they allow for the value being negative?
    //
    // You can move the bar to the middle of the widget by setting the x
coordinate of its localPosition
    // to half the width (see above) of the widget.
    //
    // Important: remember that This BarGraph component is in a different game
object than the
// Bar. So they have different RectTransforms. How do you get the
transform for the bar?
if (Min < 0)
            {
                signedDisplay = true;
                BarTransform.localPosition = new Vector3(width / 2,
                BarTransform.localPosition.y, BarTransform.localPosition.z);
            }
    }
    /// <summary>
    /// Change the value displayed in the bar graph
    /// This should both change the bar itself and also the text displayed below
    it.
/// </summary>
...
public void SetReading(float value)
    {
        ...
        // i think this is so that the rubric is satisfied?
        ...
        // TODO: Call SetWidthPercent to change the width of the bar and set its
        color
SetWidthPercent(updatedValue, color);
    ...
    public void SetWidthPercent(float value, Color c)
    {
        ...
        BarImage.color = c;
        // TODO: Change BarTransform.localScale so that its x component is scaled
        by value.
// If we're using signedDisplay, then we also want to cut the scale by a
half so we can
...
if (signedDisplay)
    ...
    private static readonly Dictionary<string, BarGraph> BarGraphTable = new
    Dictionary<string, BarGraph>();
    /// <summary>
    /// Find the bar graph stored with the specified name and return it.
    /// If there isn't one, then make one in the specified position and give it the
    specified
    /// min and max values. Then store it in the table so we can find it in the
    future.
    /// </summary>
    /// <param name="name">Name of the bar graph</param>
    /// <param name="position">Where to put it on the screen if it hasn't been made
    already</param>
/// <param name="min">Minimum value for it to display</param>
/// <param name="max">Maximum value for it to display</param>
/// <returns></returns>
public static BarGraph Find(string name, Vector2 position, float min, float
max)
    {
        // TODO: Check if we've already made a bargraph of this name. If so,
        return it.
if (BarGraphTable.TryGetValue(name, out var retBar))
        {
            return retBar;
        }
        //
        ...
        //
        // The UI system requires that all UI widgets be inside of the GameObject
        that has the Canvas component.
        // So find the canvas component
        var canvas = FindObjectOfType<Canvas>();
        // TODO: Instantiate Prefab and put it inside of the game object that has
        the canvas.
// Set its position to position and its rotation to the magic value
Quaternion.identity, which means
// "don't rotate it".
GameObject go = Instantiate(Prefab, position, Quaternion.identity,
canvas.transform); // Change null to a call to Instantiate
    ...
    // This is where we stash the prefab once we've loaded it so we don't have to
    keep loading it.
private static GameObject prefab;
    ...
    private static GameObject Prefab
    {
        get
        {
            // TODO: return prefab is null, set it to
            Resources.Load<GameObject>("BarGraph")
